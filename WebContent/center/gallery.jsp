...
<meta charset="UTF-8">
<style type="text/css">
	.img {cellpadding : 20px; margin: 20px; padding : 20px;}
	#img {cellpadding : 20px; margin: 20px; padding : 20px;}
...
<table id="notice">
    ...
    for(int i=0;i<galleryList.size();i++){
    	...
    	if(gb.getFile()!=null){
%> <td onclick="location.href='gcontent.jsp?num=<%=gb.getNum()%>&pageNum=<%=pageNum%>'" class="img" id="img"><img width = "225" height="225" src='../upload/<%=gb.getFile() %>'><%=gb.getSubject() %></td>
	<%}else{
	 %>
	 <td onclick="location.href='gcontent.jsp?num=<%=gb.getNum()%>&pageNum=<%=pageNum%>'" class="img" id="img"><img width = "225" height="225" src="../images/Jo's Bread-logo.png"><%=gb.getSubject() %></td>
	 <%}
    }
    %>
</table>
